...
public class YourSolver implements Solver<Board> {
    ...
    boolean isBarried(Point point) {
        if (board.isBarrierAt(point)) return true;
        if (board.isBulletAt(point.getX(),point.getY())) return true;
        return false;
    }
    ...
    List<Direction> getDirectionsWhereISeeEnemies(Point point) {
        ...
        for (Direction direction : Direction.onlyDirections()) {
            ...
            int count = 0;
            while (isBarried(copy) && count < SCAN_RANGE_ATTACK) {
